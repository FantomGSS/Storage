...
void overwriteFile(const char* filePath, Section* sectionKG, Section* sectionL) {
    ofstream outputFileStream;
    outputFileStream.open(filePath, ios::out);
...
void getLogsFromFile(const char* filePath, vector<string>& logs, Date startDate, Date endDate)
{
    string line;
    ifstream inputFileStream;
    inputFileStream.open(filePath, ios::in);
...
void showLogs(const char* filePath)
{
...
void calculateLosses(const char* filePath)
{
...
int main()
{
    ...
    cout << "Enter the operation you want to perform. The options are: add, remove, log, clean, losses." << endl;
    string command;
    while (command != "exit")
    {
        cout << "Command: ";
