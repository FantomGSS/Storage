...
const unsigned short int LINES_DIFFERENCE = 9;                                      ///Разликата между един и същи параметър на продукт, който се съдържа в реда от файла с продуктите.
const unsigned short int MAX_SECTIONS = 12;                                         ///Максимален брой секции от тип кг или литри, като за склада максимума е 2 * 12 = 24 секции.
const unsigned short int SCOPE_OF_PRODUCT = 2;                                      ///Един продукт с еднакво име може да се зарежда на максимум два рафта,
                                                                                    ///тоест максималното количество се пресмята: SCOPE_OF_PRODUCT * MAX_CAPACITY * MAX_COMPARTMENTS = 320.
const unsigned short int RANGE_OF_DAYS = 5;                                         ///Ако в скорошните 5 дена, срока на годност на продукта изтича, той трябва да бъде премахнат.
...
/**
    Проверка дали датата е валидна. (високосни години, броя на дните в месеците, броя на месеците в годината)
*/
...
/**
    Проверка дали мерната единица принадлежи на поддържаните от склада, съответно килограми или литри.
*/
...
/**
    Функция, която връща константа от тип Date със стойност днешна дата.
*/
...
/**
    Функция, която връща константа от тип Date със стойност дата, 
    която изчислява каква ще бъде след определения брой дни, считано от днес.
    Определеният брой дни е константата RANGE_OF_DAYS.
*/
...
/**
    ...
    Ако съществува, прочита съдържанието му и пълни редовете от файла във вектор lines.
*/
...
/**
    Функция, която по подаден символен низ (път във файловата система и името, и разширението на файла)
    променя съдържанието на файл на това място с даденото име и разширение.
    Пренаписва файла, съобразявайки се с промените в склада.
*/
...
/**
    Функция, която приема символен низ, като преди това сме напрвили проверка дали символния низ
    се състои само от цифри или сме сигурни, че се състои и го превръща в число от целочислен тип без знак.
*/
...
/**
    Функция, която приема символен низ, като преди това сме напрвили проверка дали символния низ 
    отговаря на формата на датата или сме сигурни, че отговаря на формата и го превръща в дата.
*/
...
/**
    Функция, която приема символен низ и проверява дали се състои само от цифри.
*/
...
/**
    Функция, която приема символен низ, като преди това сме напрвили проверка дали символния низ отговаря на формата на позиция 
    или сме сигурни, че отговаря и го превръща в масив от числа от целочислен тип без знак.
*/
...
/**
    Функция, която приема вектори от продукти и символни низове, като от всеки символен низ от вектора чрез алгоритъм вади по един параметър на продукт 
    и по този начин след вземането на всички параметри създава продукта и го пъха във вектора от продукти.
*/
...
/**
    Функция, която приема масив от секции и вектор от продуктии и започва да пълни секциите, 
    като за всеки продукт търси точното място, съобразявайки се с позицията му.
*/
...
/**
    Функция, която по подаден символен низ (път във файловата система и името, и разширението на файла) и
    вектор от логове, отваря файл и го попълва с тези логове, без да изтрива предната записана информация. 
*/
...
/**
    Функция, която чете от входа символен низ и го връща, като задължително 
    трябва да бъде въведено нещо, иначе дава грешка и отново трябва да се въведе символния низ.
*/
...
/**
    Функция, която приема символен низ, чийто смисъл е каква дата точно се иска и чете от входа.
    За датата има сложени проверки дали е във валиден формат и дали тя е валидна според проверката за дата.
    Отново, ако не бъде въведено нищо, както и ако не отговаря на условията, ще даде грешка и отново трябва да се въвежда дата.
*/
...
/**
    Функция, която сравнява две дати. Ако първата подадена е по-ранна от втората или еднаква с нея, то тя ще върне истина.
*/
...
/**
    Функция, която чете от входа число с плаваща запетя, проверява дали е въведено правилно и ако всичко е наред, го връща.
    Иначе се въвежда отново.
*/
...
/**
    Функция, която чете от входа параметрите на продукт, като всички са задължителни, без коментара. Там може направо празен символен низ, 
    но на другите параметри, ако нищо не се въведе или се въведе нещо грешно, ще извежда грешки и наново трябва да се въвежда.
*/
Product readProductFromInput()
{
    ...
    while (1)
    {
        expiryDate = readDateFromInput("Expiry Date: ");
        if (expiryDate == todaysDate)
        {
            cout << "This expiry date expires today. Are you sure you want to enter it ?" << endl;
            string answer;
            cout << "Answer - Yes / No: ";
            getline(cin, answer);
            ...
            {
                cout << "In that case, it would be better to enter a new date!" << endl;
            }
        }
        ...
        {
            cout << "This expiry date has already expired! Please check the date again." << endl;
        }
    }
...
/**
    Функция, която чете от входа името и количеството на продукта, който искаме да махнем. 
    Отново, ако името не се въведе, ще изведе грешка и наново ще трябва да се въвежда.
*/
...
/**
    Структура, която е имплементация на позицията на продукта - съдържа три целочислени типа без знак.
    С конструктор по подразбиране и конструктор с параметри.
*/
struct Position
{
...
/**
    Функция, която зарежда подадения продукт в склада и определя правилната му позиция,
    тъй като поредиците трябва да започват от 1.
*/
...
/**
    Функция, която разделя един продукт на два в зависимост от свободното количество, 
    което може да се заеме в отделението и определя правилната му позиция.
*/
...
/**
    Функция, която определя колко е заетото количество на два рафта в секцията.
*/
...
/**
    Функция, която открива къде се намира подадения продукт в секцията, като запазва неговата позиция.
*/
...
/**
    Функция, която по подадено позиция открива първият рафт, от който 
    започва подреждането на даден едноименен продукт с тази позиция.
*/
...
/**
    Функция, която разпределя подадения едноименен продукт в различни отделение, 
    като ако е нужно разцепване на продукта, то се прави.
*/
...
/**
    Функция, която разпределя подадения едноименен продукт където може, но ако има вероятност да се разпредели твърде хаотично, 
    то ще се добави само някаква част от него или няма да се добави избощо в зависимост от заетото количество.
*/
...
/**
    Функция, която търси къде може да се добави нов продукт, като началния рафт винаги започва от четна позиция - (0, 2, 4)
*/
...
/**
    Функция, която разпределя подадения едноименен продукт, вземайки предвид неговия срок на годност.
    Ако в склада вече има едноименен продукт с този срок, функцията се опитва да го добави в същото отделение, 
    но ако няма никакво място продукта се поставя в друго отделение, а ако има някакво място - продукта се разделя.
*/
...
/**
    Функция, която добавя подаден продукт в склада.
*/
void addProduct(Product& product, Section*& section, vector<Log>& logs, string operation)
{
    ...
    if (product.getQuantity() > allowedQuantity)                                    ///Проверка дали количеството на подадения продукт надвишава позволеното.    
    {
    ...
    if (isExisting)                                                                  ///Ако продуктът съществува, се проверява дали количеството му не надвишава позволеното,
    {                                                                                ///след като от този продукт вече има в секцията и ако има, вече му се търси място и се зарежда в склада.
    ...
    else                                                                             ///Ако продуктът не съществува, следователно търсим къде може да се постави и го зареждаме в склада.
    {
        ...
        if (startingShelf == 65535)                                                  ///Ако не е намерено място изобщо в склада.
        {
            noPlaceFound = true;
        }
        else
    ...
    if (productSuccessfullyAdded)                                                    ///Ако продуктът е добавен успешно, трябва да се направи запис на промяната в наличността на склада.
    {
        if (operation == "add")
        {
            Log log(productInfoForLog.getDateOfReceipt(), "add", productInfoForLog); ///Създава се лог от тип добавяне и се пъха във вектора от логове.
            logs.push_back(log);
            cout << "The product has been successfully added to the storage!" << endl;
        }
    }
    else if (!productSuccessfullyAdded && noPlaceFound)                              ///Ако продуктът не е добавен, понеже няма място в склада.
    {
        cout << "There are already enough different types of products. No suitable place can be found for your product." << endl;
    }
    else                                                                             ///Ако целият продуктът не е добавен успешно, понеже ще се разпредели твърде хаотично или не се е добавил изобщо.
    {
        ...
        {
            cout << "Unfortunately, this product cannot be added to the storage. There is enough of it." << endl;
        }
    }
}

/**
    Функция, която пълни във вектор сроковете на годност на всички едноименни продукти, 
    тоест само на продуктите от два рафта.
*/
...
/**
    Функция, която смята количеството на едноименните продукти с еднакви срокове на годност.
*/
...
/**
    Функция, която премахва подадения продукт от отделение.
*/
...
/**
    Функция, която премахва всички едноименни продукти, тоест изчиства цели два рафта.
*/
void removeEntireQuantity(Section*& section, vector<Log>& logs, vector<Date>& vectorExpiryDates, Date*& sortedExpiryDates, unsigned short int indexSection, unsigned short int startingShelf, Product& productInfoForLog)
{
    ...
    Log log(getTodaysDate(), "remove", productInfoForLog);                                      ///Създава се лог от тип премахване и се пъха във вектора от логове.
    logs.push_back(log);
...
/**
    Функция, която преподрежда едноименните продукти на два рафта.
*/
...
/**
    Функция, която премахва подадения продукт от склада, 
    извиквайки функцията за премахване на продукт от отделение.
*/
void removeProduct(Product& product, Section*& section, vector<Log>& logs)
{
    ...
    sort(sortedExpiryDates, sortedExpiryDates + vectorExpiryDates.size(), compareDates);                      ///Сроковете на годност се сортират, 
                                                                                                              ///за да може да се изваждат проуктите с най-скоро изтичащ срок.
    
    if (product.getQuantity() > getOccupiedQuantity(section, indexSection, startingShelf))                    ///Ако количеството, което трябва да се премахне е по-голямо от това, което е налично,
    {                                                                                                         ///се дава избор дали да се премахне цялото или ако не, трябва да се въвежда наново.
    ...
    else
    {
                    ...
                    if (section[indexSection].getShelves()[k].getCompartments()[t].getProducts().size() != 0)
                    {
                        for (int p = 0; p < section[indexSection].getShelves()[k].getCompartments()[t].getProducts().size() && !productSuccessfullyRemoved; p++)
                        {
                            ...
                            if (dateFromSection == sortedExpiryDates[v])
                            {
                                ...
                                if (quantity > product.getQuantity())                                         ///Ако количеството, което трябва да се махне, е по-малко от това, 
                                {                                                                             ///което продуктът в отделението има, неговото количество се намаля.
                                ...
                                else if (quantity == product.getQuantity())                                   ///Ако количеството, което трябва да се махне, е колкото това,
                                {                                                                             ///което продуктът в отделението има, продуктът се маха.
                                    performRemoval(section, sortedExpiryDates, quantity, productInfoForLog, indexSection, v, k, t, p);
                                    productSuccessfullyRemoved = true;
                                }
                                else                                                                          ///Ако количеството, което трябва да се махне, е повече от това, което продуктът
                                {                                                                             ///в отделението има, продуктът се маха и от количеството се изважда количеството на продукта.
                                    performRemoval(section, sortedExpiryDates, quantity, productInfoForLog, indexSection, v, k, t, p);
        ...
        if (!shelvesAreEmpty)                                                                                  ///Ако рафтовете са празни, няма нужда от пренареждане.
        {
...
/**
    Функция, която намира къде се намира продукт, прочетен от входа със стойности име и количество. 
    В какъв тип секция е, дали в тези за килограми или в тези за литри.
    Щом го намери, му се слага и параметъра за мерна единица.
*/
...
/**
    Функция, която пресмята общото количество заето от подадения продукт.
*/
...
/**
    Функция, която принтира информация за наличността в склада.
*/
...
/**
    Функция, която разчиства продуктите, които вече не са годни или предстои да им изтече срока на годност в скорошните 5 дни.
*/
                ...
                for (int p = 0; p < section[i].getShelves()[k].getCompartments()[t].getProducts().size(); p++)
                {
                        ...
                        || compareDates(dateFromSection, dayAfterFewDays))
                    {
                        ...
                        cout << "The product " << removalProduct.getName() << " with quantity " << removalProduct.getQuantity() 
                             << " at position [" << removalProduct.getPositionString() << "] has been cleared!" << endl;
...
/**
    Функция, която по подаден символен низ (път във файловата система и името, и разширението на файла)
    отваря файл, прочита съдържанието му и пълни редовете от файла във вектор lines.
*/
...
/**
    Функция, която по подаден символен низ (път във файловата система и името, и разширението на файла)
    отваря файл или ако няма такъв, го създава и по прочетени дати от входа,
    търси във файла редовете отговарящи на условието зададено чрез тях.
    Всеки ред си има дата и тази дата трябва да е между датите подадени от входа
    или поне да е равна на някоя от прочетените.
*/
...
/**
    Функция, която по подаден символен низ (път във файловата система и името, и разширението на файла)
    отваря файл или ако няма такъв, изписва, че все още няма въведени данни.
    Ако има, пресмята загубите по прочетени от входа име на продукт и цена на килограм 
    или литър като първо пресмята изхвърлените количества на продукта чрез логовете.
*/
...
/**
    Функция, която проверява дали склада е празен.
*/
...
/**
    Функция, която извиква функцията за добавяне, 
    но първо определя в кой тип секции трябва да се добави продуктът.
*/
...
/**
    Функция, която извиква функцията за премахване, 
    но първо определя от кой тип секции трябва да се премахне продуктът.
*/
...
/**
    Функция, която извиква функцията за разчистване на склада.
*/
...
/**
    Функция, която освобождава паметта от прочетените продукти от файла.
*/
...
/**
    Функция, която чрез функциите за прочитане на файла с продуктите и извличането им от него,
    ги преглежда, и съобразявайки се с техените мерни едници, ги нарежда в склада.
*/
...
/**
    Функция, която дава полезна информация за правилното използване на командите.
*/
void helpCommand()
{
    ...
    while (1)
    {
        ...
        if (command.size() > 0)
        {
                ...
                || command == "save" || command.substr(0, 7) == "save as" || command == "losses") && storageIsOpen)
            {
                ...
                else if (command == "save")                                                             //Ако командата е save, информацията от логовете се допълва във файла с логовете, 
                {                                                                                       //като след това се изчиства вектора от логове, а файла с продуктите се пренаписва.
                    completeLogFile(filePathLogs, logs);
                    logs.clear();
                    overwriteFile(filePathStorage, sectionKG, sectionL);
                }
                else if (command.substr(0, 7) == "save as" && command.size() > 8)                       //Ако командата е save as, по подаден път от входа се създава на това място нов файл,
                {                                                                                       //където ще бъде записана актуалната информация за продуктите в склада.
            ...
            if (command == "close" && storageIsOpen)                                                    ///Ако командата е close, освен склада се разчистват и векторите, 
            {                                                                                           ///които сме използвали за зареждане на склада в началото.
                storageIsOpen = false;
                products.clear();
                logs.clear();
                lines.clear();
