...
const string LOGS_PATH = "Logs.txt";                                                ///Файла, в който ще се записват логовете от промените в наличността.
const string ANSWERS[2] = { "No", "Yes" };                                          ///Позволените отговори при зададен въпрос.
const string UNITS_OF_MEASUREMENT[2] = { "kg", "l" };                               ///Позволените мерни единици.
...
void removeProduct(Product& product, Section*& section, vector<Log>& logs)
{
    ...
    sort(sortedExpiryDates, sortedExpiryDates + vectorExpiryDates.size(), compareDates);                      ///Сроковете на годност се сортират, 
                                                                                                              ///за да може да се изваждат продуктите с най-скоро изтичащ срок.
...
void calculateLosses(string filePath)
{
    ...
    else
    {
        ...
        for (int i = 0; i < lines.size(); i++)
        {
            ...
            if (indexCleaned != -1 && indexName != -1 && (lines[i].find("of") + 9 + name.size()) == indexCleaned)  // potatoes vs red potatoes...допълнителна проверка, за да не стават грешки в изчисленията
            {
...
/**
    Функция, която по подаден символен низ (път във файловата система и името, и разширението на файла)
    пренаписва файла, като запазва промените в наличността в склада. Попълват се и логовете, чрез
    функцията completeLogFile.
*/
...
/**
    Функция, която разчиства заетата памет от файловете и извежда запитване дали потребителя иска
    да запази промените направени от него, преди да изчисти данните.
*/
...
/**
    Функция, която по подаден символен низ (път във файловата система и името, и разширението на файла)
    създава файл на това място и записва актуалната информация за продуктите в склада.
*/
